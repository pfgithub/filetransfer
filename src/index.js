...
if(resips.length > 1) {
	console.log("Too many ip addresses. Launch with a listed env command to select correct address.");
	console.log("Address will be used to show links and QR codes to download files.");
	console.log();
...
function randomName() {
	return new Promise((resolve) => {
		crypto.randomBytes(16, async (err, buffer) => {
			let str = buffer.toString("hex").replace(/[0-9]/g,
...
function downloadFile(mode, get) {
	return async (req,res,next) => {
		...
		let files;try{files = await fs.readdir(foldername)} catch(e){
			res.status(404);
		...
		if(mode === "view") res.sendFile(filepath); else res.download(filepath);
	};
}
